...
To achieve this we implemented PLASMA's DPOTRF routine and measured the processing time of its execution on two, four, eight, ... , 48 cores. For time measuring we used the library PAPI [2].
All measurements were done on a system with an x86_64 architecture, 48 CPUs, one thread per core, AMD Opteron(tm) Processor 6174 with a theoretical peak performance of 2200.00 MHz. Furthermore there are 8 NUMA nodes, which combine six cores per each node.
In the end we compared PLASMA's DPOTRF runtimes to the sequential cholesky decomposition implemented in LAPACK [3] and calculated absolute and relative speedups.
